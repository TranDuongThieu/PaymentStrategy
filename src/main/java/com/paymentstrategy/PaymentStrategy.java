...
//            PaypalPayment paypalStrategy = new PayPalPay();
            ...
            switch (paymentMethod) {
                case "1":
                    procPay.setDetailStrategy(new AccountDetail());
                    procPay.setPayStrategy(new PaypalIsPay());
                    break;
                case "2":
                    procPay.setDetailStrategy(new CreditCartDetaIl());
                    procPay.setPayStrategy(new MomoIsPay());
                    break;
                case "3":
                    procPay.setDetailStrategy(new AccountDetail());
                    procPay.setPayStrategy(new ZaloIsPay());
                    break;
            }
            procPay.getDetail();
            Thread.sleep(1000);
